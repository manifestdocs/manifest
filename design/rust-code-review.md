...
| ~~`list_features` / `list_project_features` fetch *all* rows then paginate in Rust.~~ | ~~Memory/time usage grows linearly with the dataset, defeating pagination.~~ | ✅ **FIXED** – Added `get_all_features_paginated()` and `get_features_by_project_paginated()` that push `LIMIT`/`OFFSET` into SQL. |
