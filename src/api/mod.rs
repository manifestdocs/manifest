...
pub use clerk::{ClerkClaims, ClerkError, ClerkVerifier};
pub use clerk_middleware::{
    clerk_auth_middleware, optional_clerk_auth_middleware, AuthenticatedUser, ClerkAuthState,
};
...
/// Create the API router with Clerk authentication for cloud mode.
pub fn create_router_with_clerk(db: Database, verifier: ClerkVerifier) -> Router {
    let config = SecurityConfig::from_env();
    let clerk_state = ClerkAuthState {
        verifier,
        db: db.clone(),
    };
